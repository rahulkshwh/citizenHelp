...
export default function TodayPage() {
  const [greeting, setGreeting] = useState("Good day! ☀️");
  const [medicines, setMedicines] = useState<Medicine[]>([]);
  const [scamTip, setScamTip] = useState({ title: "", tip: "" });

  useEffect(() => {
    setGreeting(getGreeting());
    setMedicines(loadMedicines());
    setScamTip(getDailyScamTip());
  ...
  return (
    <div className="today-page">
      ...
      <section className="section-card scam-tip-section" aria-labelledby="scam-tip-heading">
        <h2 id="scam-tip-heading">🛡️ Daily Scam Tip</h2>
