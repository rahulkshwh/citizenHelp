...
export default function MedicinesPage() {
  ...
  useEffect(() => {
    ...
    const handleUpdate = () => setMedicines(loadMedicines());
    window.addEventListener("saathi-medicines-change", handleUpdate);
    return () => window.removeEventListener("saathi-medicines-change", handleUpdate);
  }, []);

  function handleToggle(id: string, name: string) {
    const updated = toggleMedicineTaken(id);
    setMedicines(updated);
    const med = updated.find((m) => m.id === id);
    const taken = med ? isMedicineTakenToday(med) : false;
    setStatusMessage(`${name} marked as ${taken ? "taken" : "not taken"}.`);
  }
  ...
  function handleAddMedicine(e: FormEvent<HTMLFormElement>) {
    ...
    setStatusMessage(`Added ${newMed.name} to your schedule.`);
  }
  ...
  function handleResetDefaults() {
    if (!confirm("Reset medicine schedule to recommended defaults?")) return;
    saveMedicines(DEFAULT_MEDICINES);
    setMedicines(DEFAULT_MEDICINES);
    setStatusMessage("Schedule reset to defaults.");
  }
  ...
  return (
    <section className="medicines-page">
      ...
      <div className="medicines-actions">
        ...
        <button type="button" className="setting-button" onClick={handleResetDefaults}>
          Reset Defaults
        </button>
      </div>
